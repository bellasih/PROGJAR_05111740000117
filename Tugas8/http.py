...
class HttpServer:
	...
	def proses(self,data):
		...
		try:
			...
			if (method=='POST'):
				res = requests[-1]
				result = res.split("=")
				return self.http_post(object_address,all_headers,result[1])
			else:
	...
	def http_post(self,object_address,headers,result):
		headers={}
		headers['Content-type']="text/html"
		return self.response(200,'OK','<p>'+result+'</p>',headers)
